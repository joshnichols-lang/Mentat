...
- **Prediction Markets Tab:** Tag-based market browser using Polymarket's actual eventTags from Gamma API. Features top 30 most popular tags (Trump, Bitcoin, Sports, etc.) extracted from market eventTags with frequency-based sorting. Single-state tag selection eliminates category/filter state coordination issues. Includes search functionality and trading modal (Polymarket integration). **Recent Fix (Nov 2, 2025):** Replaced keyword-based category filtering with API-driven tag system to fix UI "messing up" when switching categories and ensure short-term Bitcoin/Ethereum markets are visible.
