...
- **Mandatory Protective Orders (Oct 20, 2025):** CRITICAL SAFETY: Server-side validation REJECTS any trading strategy that would leave existing positions without stop loss orders. AI system prompt enforces that ALL positions MUST have stop_loss actions in EVERY response. This prevents positions from ever being unprotected, even for a single monitoring cycle. Strategy execution fails immediately if any position lacks stop loss coverage.
