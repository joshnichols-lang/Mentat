...
**Cross-Chain Deposits:** Router Nitro Widget integration enables deposits from 30+ chains (Ethereum, Arbitrum, Polygon, Base, Optimism, etc.) directly to embedded Hyperliquid wallet. Widget opens in popup with pre-filled destination address. URL-encoded parameters ensure safety. Popup blocker detection provides user feedback.
...
**Multi-Chain Balance Display:** Header features ChainSwitcher component showing real-time balances across Solana (SOL + USDC), Arbitrum (ETH + USDC), and Hyperliquid (Portfolio Value + Free Margin). Total balance tooltip displays per-chain breakdown. Dropdown allows switching between SOL and Perps chains with balance details. Low free margin warning badge appears when Hyperliquid margin falls below 20%. 30-second auto-refresh via useWalletBalances hook.
...
**Multi-Chain Wallet System:** Non-custodial architecture where users authenticate with external wallets (MetaMask/Phantom), but platform generates a separate BIP39-derived multi-chain wallet for deposits and trading. **Critical security design**: Seed phrase shown once to user, then deleted—never stored in database. Only public addresses (Solana, EVM, Hyperliquid) stored for balance queries. Users deposit funds from connected wallet or external sources into generated wallets.
**Hyperliquid API Wallet Architecture:** Platform creates a separate API wallet (from different seed) for Hyperliquid trading. User signs EIP-712 `approveAgent` transaction in-app using connected wallet, authorizing API wallet to trade on behalf of their generated Hyperliquid wallet. API wallet has zero withdrawal permissions—can only sign trading transactions. **Only the API wallet private key is stored (encrypted)** in the database. All trades execute under 1fox's referral code for fee earnings. Users maintain full control: can withdraw by signing with connected wallet, can revoke API wallet anytime from Hyperliquid UI.
**AI Integration:**
