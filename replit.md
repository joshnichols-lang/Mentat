...
- **CRITICAL Trade Execution Bug Fixes (RESOLVED):**
  ...
  - **Trigger Fallback Logic (FINAL FIX):** Modified evaluateStrategyTriggers in strategyTriggers.ts to default shouldCallAI=true when strategies lack trigger configs (indicatorConfig/orderFlowConfig/marketProfileConfig). New strategies now properly execute time-based AI monitoring until they build enough history for event-driven triggers. Added 'time_based_fallback' to triggeredBy array with clear logging for observability. Fixes catch-22 where new strategies couldn't enter first trade due to no price history → no signals → no AI calls.
