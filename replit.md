...
- **Custom Rules Priority System (Oct 21, 2025):** User-defined custom rules in trading strategies OVERRIDE hardcoded defaults when more conservative. For example: if user specifies "only 3:1 R:R setups", AI enforces 3:1 instead of default 1.5:1. If custom rules specify "max 2% risk per trade", AI uses 2% instead of strategy's configured percentage. Custom rules represent user's trading philosophy and take precedence over generic constraints. Hardcoded rules serve as defaults when custom rules are general (e.g., "focus on momentum").
