...
**Per-Symbol Entry Limit (Oct 19, 2025):** Implemented `MAX_ENTRY_ORDERS_PER_SYMBOL = 3` constraint to prevent excessive scaled entries. System now groups entry actions by symbol, ranks by conviction (reasoning length), and keeps only top 3 entry orders per symbol. Combined with max positions limit (5 symbols), this ensures reasonable total order counts (~27 max vs previous 37+).
