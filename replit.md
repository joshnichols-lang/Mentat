...
- **x402 Micropayment Infrastructure:** Coinbase's HTTP-based payment protocol enabling pay-as-you-go AI usage using USDC on Base blockchain. Provides fallback monetization when tier quotas are exhausted without requiring tier upgrades. AI endpoints check tier quota first; if exhausted, check x402 balance; if neither available, return HTTP 402 Payment Required with payment details. Users deposit USDC to platform wallet (environment variable X402_PLATFORM_WALLET), which is verified on-chain and credited to their x402 balance. **Dynamic Pricing Model**: Users pay exactly 2x the actual AI API cost (100% markup). Typical costs range from $0.002-0.02 per call depending on model used (Grok 4 Fast ~$0.002, GPT-4o ~$0.05). Fair, transparent, and ensures profitability while remaining competitive. Database schema includes x402Balance field in users table and x402Transactions table tracking deposits, payments, refunds, and cost breakdown (actualAiCost, markup, totalCharge) in transaction metadata. Payment processing is atomic - balance deducted only after successful AI response using row-level locking to prevent race conditions. API endpoints at /api/x402/* provide balance checking, deposit recording, transaction history, and payment statistics. SDK packages: x402, x402-express, @coinbase/x402. Zero protocol fees, ~$0.001 gas on Base, 2-second settlement. Status: Development/testing phase - blockchain verification placeholder currently returns true pending production RPC integration.
