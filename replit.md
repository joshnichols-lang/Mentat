...
- **Dual AI Modes (Oct 21, 2025):**
  - **Conversational Mode (No Strategy Active):** Pure Grok with no constraints. AI responds naturally to any question (math, science, current events, etc.) with full access to Hyperliquid account data (portfolio value, available balance, positions, orders). Activated when: (1) no strategy active + informational keywords (margin/balance/position), OR (2) no strategy active + general questions, OR (3) meta/verification questions (asking AI to verify/check its work).
  - **Trading Mode (Strategy Active OR Execution Keywords):** "Mr. Fox" trading agent with structured JSON responses. Activated when: (1) strategy active (any prompt), OR (2) execution keywords detected (buy/sell/close/long/short).
  - **Classification Fix (Oct 21, 2025):** Added priority detection for meta/verification questions (e.g., "double check", "verify", "make sure") to prevent triggering trading mode when users ask the AI to verify its own work or calculations. These questions now always use conversational mode regardless of active strategy.
...
- **Custom Rules Priority System (Oct 21, 2025):** User-defined custom rules in trading strategies guide AI behavior and risk management. Users define their trading philosophy through strategy descriptions and custom rules (e.g., "only 3:1 R:R setups", "max 2% risk per trade", "only enter on confirmed breakouts"). The AI learns and adapts to each strategy's unique approach through conversation and execution feedback.
- **Strategy Context Awareness (Oct 19, 2025):** AI prompts now include active trading strategy details (name, description, risk %, max positions, leverage, timeframe, preferred assets, custom rules) when a strategy is selected. This enables the AI to understand and reference strategy constraints when users ask questions about it, ensuring responses align with the active strategy's rules. Comprehensive logging added (Oct 20, 2025) to diagnose strategy context delivery: frontend logs active mode details in browser console, backend logs strategy fetch status and full details in server logs. **Critical Bug Fix (Oct 20, 2025):** Moved strategy context fetching before classification check in `server/tradingAgent.ts`. Previously, when AI classified user questions as "GENERAL" (e.g., "what is the current strategy?"), the function returned early before fetching strategy details, preventing AI from knowing about active strategy. Now strategy context is fetched first and included in both trading AND general question prompts, allowing AI to answer strategy-related questions correctly.
- **Position Display Fix (Oct 20, 2025):** Fixed PnL percentage showing 0.00% by adding `returnOnEquity` field mapping in `server/hyperliquid/client.ts`. The Hyperliquid API provides this as a decimal (e.g., 0.0195 for 1.95%), and frontend multiplies by 100 for display.
...
- **Active Mode:** Enables automatic monitoring; AI autonomously develops and executes trading strategies guided by user-defined custom rules. Critical safety constraints remain ($10 minimum notional, mandatory protective brackets, 20% Terminal Safety Guard max price distance, asset-specific leverage caps). No hardcoded R:R ratio - users define their own risk/reward requirements through strategy custom rules.
