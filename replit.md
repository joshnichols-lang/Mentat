...
## Recent Changes
**November 8, 2025:**
- **CRITICAL Position Sizing Fix:** Fixed AI position sizing calculations that were using `balance × leverage` instead of risk-based formulas. Added explicit position sizing instructions to AI prompt (tradingAgent.ts lines 570-630) with step-by-step formula: Risk Amount → SL Distance → Notional Value → Size. Clarified leverage affects margin requirement, not position size calculation (Size = Notional / Entry Price). Includes three worked examples and validation checklist. Positions now risk exactly the specified percentage (1-2%) per trade regardless of leverage used. Prevents dangerously oversized positions that could risk entire account.
- **CRITICAL Monitoring Loop Wiring Fix (FINAL RESOLUTION):** Fixed the root cause preventing new strategies from executing trades. The monitoring loop (monitoringService.ts) had a restrictive conditional that only called `evaluateStrategyTriggers` for strategies with `triggerMode='indicator'` or `'hybrid'`. New strategies with `triggerMode='time_based'` or `strategyType='price_action'` were bypassed to old generic trigger code that returned `shouldCallAI=false`. Changed condition from checking trigger mode to simply `if (strategyConfig)` - now ALL strategies use `evaluateStrategyTriggers` with its built-in fallback logic. Strategies WITH configs use event-driven triggers (cost savings), strategies WITHOUT configs default to time-based monitoring (`shouldCallAI=true`). Moved candle fetching outside nested conditions so all strategies get price data. Added logging to show strategy type and trigger mode for observability. This completes the three-layer fix: frontend trigger config → backend fallback → monitoring loop wiring.
...
- **CRITICAL Trade Execution Bug Fixes:**
  ...
  - **Trigger Fallback Logic:** Modified evaluateStrategyTriggers in strategyTriggers.ts to default shouldCallAI=true when strategies lack trigger configs (indicatorConfig/orderFlowConfig/marketProfileConfig). New strategies now properly execute time-based AI monitoring until they build enough history for event-driven triggers. Added 'time_based_fallback' to triggeredBy array with clear logging for observability.
