...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Watchlist routes
  app.get("/api/watchlist", requireVerifiedUser, async (req, res) => {
    try {
      const userId = getUserId(req);
      const symbols = await storage.getWatchlistSymbols(userId);
  ...
  app.post("/api/watchlist", requireVerifiedUser, async (req, res) => {
    try {
      const userId = getUserId(req);
      const { symbol, displayName, binanceSymbol, tradingViewSymbol, sortOrder } = req.body;
      
      const watchlistSymbol = await storage.addWatchlistSymbol(userId, {
        symbol,
        displayName,
        binanceSymbol,
        tradingViewSymbol,
      ...
      console.error("Error adding watchlist symbol:", error);
      res.status(500).json({
        success: false,
  ...
  app.delete("/api/watchlist/:symbol", requireVerifiedUser, async (req, res) => {
    try {
      const userId = getUserId(req);
      const { symbol } = req.params;
      
      await storage.removeWatchlistSymbol(userId, symbol);
      
      res.json({
        success: true
      });
    } catch (error: any) {
      console.error("Error removing watchlist symbol:", error);
      res.status(500).json({
        success: false,
        error: "Failed to remove watchlist symbol"
      });
    }
  });
