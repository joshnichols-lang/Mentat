...
[Prioritized list of what the user should do next]`;
  ...
  app.post("/api/wallets/embedded", isAuthenticated, async (req, res) => {
    try {
      ...
      // Automatically create encrypted Hyperliquid API credentials for trading
      // This ensures users can immediately activate trading mode after wallet creation
      ...
      await storage.createApiKey(userId, {
        ...
        metadata: {
          ...
          autoGenerated: true,
          purpose: "embedded_wallet_trading",
        } as any,
      });
      
      console.log(`[Wallet] Created embedded wallets and encrypted Hyperliquid credentials for user ${userId}`);
