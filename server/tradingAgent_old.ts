...
export async function processTradingPrompt(
  ...
  try {
    ...
    if (strategyId) {
      ...
      console.log(`[Trading Prompt] ℹ️ No strategyId provided (null/undefined) - proceeding without strategy constraints`);
    }
...
${params.customRules ? `- Custom rules: ${params.customRules}\n\nThese custom rules represent the user's trading philosophy. Follow them strictly when making trading decisions.` : ''}

When the user asks you to trade, analyze markets, or modify strategy parameters, you can generate trading actions in your response.`;
    }
...
Use this history to understand the user's recent questions and any strategy modifications they've requested.`;
    }

    const messages: AIMessage[] = [
      {
        role: "system" as const,
        content: `You are Grok, an AI assistant helping users with their Hyperliquid trading account. You respond naturally and conversationally to all questions.${strategyContext}${conversationContext}
...
You can answer ANY question - math, science, current events, trading, market analysis, or casual conversation. When appropriate based on the user's request and context, you can include trading actions in your response using the JSON format below.

WHEN TO INCLUDE TRADING ACTIONS:
- User explicitly asks you to trade, buy, sell, or close positions
- User asks you to analyze markets and suggests a trade setup
- User modifies the active strategy (e.g., "use 2 ATR stops instead of 1")
- You're autonomously monitoring and find a trading opportunity aligned with the active strategy

HOW TO FORMAT YOUR RESPONSE WITH TRADING ACTIONS:
You must respond with ONLY valid JSON. No other text before or after. Format:
